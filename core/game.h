...
to look into this) if the strategy is identical to knuth’s.
      */
      ...
      state_ = newState<Ludii::LudiiStateWrapper>(
    ...
    setFeatures(outFeatures, turnFeatures, geometricFeatures, history,
                randomFeatures, oneFeature);

    state_->Initialize();
  }

  template <typename T> std::unique_ptr<State> newState(int seed) {
    auto r = std::make_unique<T>(seed);
  ...
  void addPlayer(std::shared_ptr<mcts::MctsPlayer> player,
    ...
    if (seqlen) {
      rnnStateSize =
          player->rnnStateSize().at(0) * player->rnnStateSize().at(1);
    }
    ...
    auto rnnInitialState = tube::EpisodicTrajectory(
        "rnn_initial_state",
        {rnnStateSize * state_->GetRawFeatureSize().at(1) *
    ...
    auto v = tube::EpisodicTrajectory("v", addseq({1}), torch::kFloat32);
    ...
    send = {feat.buffer, pi.buffer, piMask.buffer, v.buffer};
...
// std::unique_lock<std::mutex> lk(terminateMutex_);
 ...
 private:
  ...
  std::vector<std::vector<float>> rnnState_;
};
