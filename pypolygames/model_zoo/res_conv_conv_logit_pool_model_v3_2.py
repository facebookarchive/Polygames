...
class ResConvConvLogitPoolModelV3_2(torch.jit.ScriptModule):
    __constants__ = [
        "c_prime",
        "h_prime",
        "w_prime",
    ...
    def forward(self, x: torch.Tensor):
        ...
        for resnet, poolblock in zip(self.resnets, self.poolblocks):
        ...
        if self.predict_pi_logit is not None:
            ...
            return {"v" : v, "pi_logit" : pi_logit, "predict_pi_logit" : predict_pi_logit}
        else:
            return {"v" : v, "pi_logit" : pi_logit}
    ...
    def loss2(
        ...
        pred_v, pred_logit = output["v"], output["pi_logit"]

        pi_mask = pi_mask.view(pred_logit.shape);
        pred_logit = pred_logit * pi_mask - 400 * (1 - pi_mask)
        if self.predicts > 0:
            pred_predict_logit = output["predict_pi_logit"]
            predict_pi_err = (F.mse_loss(pred_predict_logit, predict_pi, reduction="none") * predict_pi_mask).flatten(2).sum(2).flatten(1).mean(1)

        v_err = -(nn.functional.log_softmax(pred_v, dim=1) * v).sum(1)
        pred_log_pi = nn.functional.log_softmax(pred_logit.flatten(1), dim=1).view_as(pred_logit) * pi_mask
        pi_err = -(pred_log_pi.flatten(1) * action_pi.flatten(1) * v.narrow(1, 0, 1)).sum(1)
