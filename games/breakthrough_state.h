...
class StateForBreakthrough : public State, BTBoard {
 public:
  ...
  void findActions(int color) {
    ...
    for (int i = 0; i < nb; i++) {
      ...
      if (moves[i].x1 == x - 1)
        ...
        dir = 1;
      _NewlegalActions.emplace_back(i, dir, x, y);
    }
  }
  ...
  virtual float getReward(int player) const override {
    if (_status == GameStatus::player0Win) {
      return player == 0 ? 1.0 : -1.0;
    } else if (_status == GameStatus::player1Win) {
      return player == 1 ? 1.0 : -1.0;
    }
    int score = countPieces(player) - countPieces(opponent(player));
    int diff = score - prevscore[player];
