...
struct BatchExecutor {
  ...
  float runningAverageScore = 0.0f;
  std::chrono::steady_clock::time_point lastPrintScore = std::chrono::steady_clock::now();
  ...
  std::list<GameState>::iterator addGame(std::list<GameState>::iterator at) {
    ...
    if (randint(250) == 0) {
      //fmt::printf("runningAverageGameSteps is %g\n", runningAverageGameSteps);
      switch (randint(2)) {
      case 0:
  ...
  using stateCallback = void(BatchExecutor::*)(GameState*, int currentPlayerIndex, size_t index) const;

  void actPrepareRnn(GameState* gameState, int currentPlayerIndex, size_t index) const {
    size_t slot =
    ...
    const_cast<torch::Tensor&>(actRnnState[index]) = std::move(gameState->rnnState[slot]);
  ...
  void actPrepareForward(GameState* gameState, int currentPlayerIndex, size_t index) const {
  ...
  void actResult(GameState* gameState, int currentPlayerIndex, size_t index) const {
    ...
    static thread_local std::mt19937_64 tlrng(std::random_device{}());

    if (forwardPlayer) {
      forwardPlayer->batchResult(index, *state, pival);

      gameState->rnnState[slot] =
          std::move(pival.rnnState);
...
//      while (true) {
//        double num = std::uniform_real_distribution<double>(0.0f, 1.0f)(tlrng);
//        for (bestAction = 0; bestAction < (int)x.size() - 1; ++bestAction) {
//          num -= x[bestAction];
//          if (num < 0) {
//            break;
//          }
...
//      }
      ...
      std::unordered_map<int, float> maxval;
      std::unordered_map<int, float> sumval;
      std::unordered_map<int, int> count;
      for (size_t i = 0; i != x.size(); ++i) {
        auto& a = state->GetLegalActions().at(i);
        float& mv = maxval[a.GetX()];
        mv = std::max(mv, x[i]);
        sumval[a.GetX()] += x[i];
        ++count[a.GetX()];
      }
//      for (size_t i = 0; i != x.size(); ++i) {
//        auto& a = state->GetLegalActions().at(i);
//        x[i] = x[i] / sumval[a.GetX()] * maxval[a.GetX()];
//      }

      if (std::uniform_int_distribution<int>(0, 30)(tlrng) == 0) {
        gameState->randMoveCount = std::uniform_int_distribution<int>(1, 6)(tlrng);
      }
      if (gameState->randMoveCount > 0) {
        --gameState->randMoveCount;
        for (size_t i = 0; i != x.size(); ++i) {
          auto& a = state->GetLegalActions().at(i);
          x[i] = 1.0f / std::sqrt((float)count[a.GetX()]);
          //printf("random uniform\n");
        }
      }

      if (std::uniform_int_distribution<int>(0, 1)(tlrng) == 0) {
        for (size_t i = 0; i != x.size(); ++i) {
          //x[i] = std::exp(x[i] * 4);
          x[i] = std::exp(x[i] * 4) - 1.0f;
        }
      }
...
#ifdef OPENBW_UI
      for (auto& [k, v] : maxval) {
        printf("action channel %d sum %g max %g n %d\n", k, sumval[k], v, count[k]);
      }
#endif
...
//      double best = 0.0f;
//      for (size_t i = 0; i != x.size(); ++i) {
//        //float v = std::exp(x[i] * 6);
//        //auto& a = state->GetLegalActions().at(i);
//        //if (a.GetX() != 0 && a.GetX() != 5 && a.GetX() != 6 && a.GetX() != 3 && a.GetX() != 4) continue;
//        //if (count[0] > 0 && a.GetX() != 0) continue;
//        double v = x[i];
//        v = std::uniform_real_distribution<double>(0.0f, v)(tlrng);
//        if (v > best) {
//          best = v;
//          bestAction = i;
//        }
//      }
      ...
      if (std::uniform_int_distribution<int>(0, 30)(tlrng) == 0) {
        bestAction = std::uniform_int_distribution<int>(0, (int)state->GetLegalActions().size() - 1)(tlrng);
        //printf("random action %d\n", bestAction);
      }

      //printf("sampled action %d (%d %d %d) with val %g\n", bestAction, state->GetLegalActions().at(bestAction).GetX(), state->GetLegalActions().at(bestAction).GetY(), state->GetLegalActions().at(bestAction).GetZ(), x[bestAction]);
      //printf("sampled action %d (%d %d %d) with val %g (rolled %g)\n", bestAction, state->GetLegalActions().at(bestAction).GetX(), state->GetLegalActions().at(bestAction).GetY(), state->GetLegalActions().at(bestAction).GetZ(), x[bestAction], best);
      ...
      gameState->rnnState[slot] =
          std::move(mctsResult.at(index).rnnState);
...
#ifdef OPENBW_UI
    auto x =
        state->GetLegalActions().at(bestAction);
...
//        }
      ...
      if (forwardPlayers.at(playerIndex)) {
        ...
        mctsResult = mctsPlayers.at(playerIndex)->actMcts(playerActStates, actRnnState, trees);
      }
          ...
          for (size_t i = 0; i != currentPlayerStates; ++i) {
            ...
            if (gameState->allowRandomMoves.at(slot)) {
              ...
              if (std::uniform_real_distribution<float>(0, 1.0f)(rng) < x) {
                mctsResult.at(offset + i).bestAction = randint(state->GetLegalActions().size());
      ...
      if (&*game->playerGame_.at(playerIndex) != game) {
        if (devPlayer->rnnSeqlen()) {
          ...
          for (size_t pi = 0; pi != statePlayerSize.size(); ++pi) {
            ...
            for (size_t i = 0; i != currentPlayerStates; ++i) {
              ...
              if (!gameState->rnnState2.at(slot).defined()) {
                gameState->rnnState2[slot] = torch::zeros(devPlayer->rnnStateSize());
              }
          ...
          std::vector<torch::Tensor> nextRnnState = devPlayer->nextRnnState(states, actRnnState);
      ...
      mctsOption = mctsPlayers[playerIndex] ? &mctsPlayers[playerIndex]->option() : nullptr;
    ...
    while (!states.empty() && !game->terminate_) {
      ...
      for (auto i = states.begin(); i != states.end();) {
        ...
        if (completed) {
          ...
          for (size_t idx = 0; idx != players_.size(); ++idx) {
            fmt::printf("state->getScore(idx) is %g\n", state->getScore(idx));
            runningAverageScore = runningAverageScore * 0.99f + state->getScore(idx) * 0.01f;
          }

          if (end - lastPrintScore >= std::chrono::seconds(10)) {
            lastPrintScore = end;
            fmt::printf("Running average game score: %g\n", runningAverageScore);
          }
        }
        ...
        if ((isForward && seqlen > 0) || completed) {
          for (size_t slot = 0; slot != players_.size(); ++slot) {
            ...
            if (completed) {
              ...
              if ((int)i->pi[slot].size() < seqlen * 16 + 1 || i->history.empty() || i->history.back().turn != (int)slot) {
                continue;
              }
              ...
              lastpredv = i->predV[slot].back().item<float>();
              ...
              float sum = 0.0f;
              for (auto& v : i->reward[slot]) {
                sum += v;
              }
              //fmt::printf("%s sequence end with cumulative reward %g\n", players_[dstp]->getName(),
              //            sum);
//              fmt::printf("%s sequence end with reward %g\n", players_[dstp]->getName(),
//                          result_[dstp]);
            }
...
#ifdef OPENBW_UI
            std::cout << " rewards is " << torch::stack(rewards) << "\n";
#endif
...
//              i->pi[slot][n] *= 0;
//              i->pi[slot][n] += result_[slot];
//            }
            // if ((actorPlayers[dstp]->getModelId() == "dev" || result_[dstp] >
            // 0) && i->feat[slot].size() > 0) {
            if (actorPlayers[dstp]->getModelId() == "dev" && i->feat[slot].size() > 0) {
              if (seqlen) {
                for (size_t n = 0; n != i->feat[slot].size(); ++n) {
                  if ((seq.feat.size() + n) % seqlen == seqlen - 1) {
                    game->rnnInitialState_[dstp].pushBack(i->rnnStates[slot].at(n));
                  }
                }
                ...
                if (isForward) {
                  addseq(i->actionPi[slot], seq.actionPi, game->actionPi_[dstp]);
                }
                ...
                addseq(rnnStateMask, seq.rnnStateMask, game->rnnStateMask_[dstp]);
              } else {
              ...
              if (game->predictEndState || game->predictNStates) {
                ...
                for (size_t m = 0; m != i->history.size(); ++m) {
                  ...
                  if (seqlen) {
                    ...
                    addseq({mask}, seq.predictPiMask, game->predictPiMask_[dstp]);
                  } else {
...
void Game::mainLoop() {
  threads::setCurrentThreadName("game thread " + std::to_string(common::getThreadId()));
  threads::init(10);
