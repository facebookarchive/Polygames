...
PYBIND11_MODULE(polygames, m) {
  ...
  py::class_<Game, tube::EnvThread, std::shared_ptr<Game>>(m, "Game")
      ...
      .def(py::init<std::string, int, int, bool, bool, bool, bool, bool, int, int,
  ...
  py::class_<ForwardPlayer, ActorPlayer, std::shared_ptr<ForwardPlayer>>(m, "ForwardPlayer")
      .def(py::init<>(), py::call_guard<py::gil_scoped_release>());

  py::class_<ModelManager, std::shared_ptr<ModelManager>>(
      m, "ModelManager")
      .def(py::init<int,
                    const std::string&,
                    int,
      ...
      .def("set_is_tournament_opponent",
           &ModelManager::setIsTournamentOpponent)
      ...
      .def("start_replay_buffer_server",
           &ModelManager::startReplayBufferServer)
      .def("start_replay_buffer_client",
           &ModelManager::startReplayBufferClient)
      .def("remote_sample", &ModelManager::remoteSample);

  py::class_<SampleResult>(m, "SampleResult")
      .def("get", &SampleResult::get);
