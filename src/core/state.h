...
struct FeatureOptions {
  ...
  bool turnFeaturesSingleChannel = false; // do we add a feature for turn (deprecated, use turnFeaturesMultiChannel instead)
  bool turnFeaturesMultiChannel = false; // do we add a feature for turn/player(color) (one channel for each player, 0 for other players, 1 for current player)
...
class State {
 public:
  ...
  virtual int getNumPlayerColors() const {
    throw std::runtime_error("getNumPlayerColors is not implemented for this game");
    return 0;
  }
  ...
  virtual float getScore(int player) const {
    return 0.0f;
  }
