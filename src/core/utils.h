...
inline void getLegalPi(const State& state,
  ...
  for (size_t i = 0; i != legalActions.size(); ++i) {
    ...
    // TODO FIX THIS
    // to make this work outside mcts:
    //   revamp how games add actions to detect at add time how many actions share locations
    //   divide the policy value here by the number of actions
    // we exchange with -400 (exp(-400) ~ 0) because:
    //  - two actions A and B can share the same policy output location
    //  - the NN will then be trained to output the sum of their policy values in that location
    //  - if we give both actions that policy output, there will be a bias in the MCTS towards exploring
    //     A and B, and the sum of all policy values will be > 1
    //  - instead, we give the sum to one action and 0 to the others, preserving the sum of probabilities
    //  - the MCTS must compensate for this by forcefully visiting both A and B whenever A or B is visited
    //      other algorithms are unlikely to support multiple actions sharing the same policy output
    //  - it would be equivalent to divide pi by the number of actions that share this policy value (2 in
    //      this case), but it would be slower as we don't know beforehand how many that is
    // this will set the source tensor to all 0s (it shouldn't be needed for anything else)
  }
}
...
template<typename T>
inline static void softmax_(T begin, T end) {
  if (begin == end) {
    return;
  }
  float max = *begin;
