...
to look into this) if the strategy is identical to knuth’s.
    ...
    setFeatures(outFeatures, turnFeaturesSingleChannel, turnFeaturesMultiChannel, geometricFeatures, history,
...
//    }
    ...
    auto v = tube::EpisodicTrajectory("v", addseq({player->vOutputs()}), torch::kFloat32);
    auto predV = tube::EpisodicTrajectory("pred_v", addseq({player->vOutputs()}), torch::kFloat32);
