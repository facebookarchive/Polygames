...
class ServerImpl {
  ...
  bool rdmaKeepalive(rdma::Endpoint remoteEp) {
    ...
    for (auto i = rdmaClients.begin(); i != rdmaClients.end(); ++i) {
      if (i->remoteEp == remoteEp) {
        //fmt::printf("keepalive: rdma client %d:%d found, timestamp updated\n", remoteEp.lid, remoteEp.qpnum);
  ...
  std::optional<RDMAModelInfo> rdmaGetModel(rdma::Endpoint remoteEp, std::string_view modelId) {
    try {
      std::unique_lock rl(rdmaMut);
      rdma::Endpoint localEp;
      for (auto i = rdmaClients.begin(); i != rdmaClients.end(); ++i) {
          ...
          if (n < 500 && i->second.rdmaSerializing.exchange(true)) {
            ...
            if (false) {
              ...
              double t1 = std::chrono::duration_cast<
                              ...
                              .count();
              fmt::printf(
                  "State dict serialized and RDMA buffer updated in %gms, %gM (checksum %#x)\n",
                  t1,
                  storage.size() / 1024.0 / 1024.0, checksum);
            } else {
              i->second.rdmaBuffer_01 = std::move(buffer);
              i->second.rdmaBufferStorage_01 = std::move(storage);

              storage = std::move(s.buf);
              buffer = rdmaContext->createBuffer((void*)storage.data(), storage.size());
              ...
              double t1 = std::chrono::duration_cast<
                              ...
                              .count();
              fmt::printf(
                  "State dict serialized and RDMA buffer created in %gms, %gM (checksum %#x)\n",
                  t1,
                  storage.size() / 1024.0 / 1024.0, checksum);
            }
 ...
 public:
  std::function<void(const std::unordered_map<std::string, torch::Tensor>)> onTrainData;
  ...
  void start(std::string_view endpoint) {
    ...
    define("requestCompressedStateDict",
           &ServerImpl::requestCompressedStateDict);
  ...
  void updateModel(const std::string& id,
    ...
    if (i.second) {
      i.first->second.version = std::uniform_int_distribution<int>(0, 10000)(rng) * 1000;
...
class ClientImpl {
  ...
  struct Bandit {
    ...
    float sample(std::string name, float weight = 1.0f) {
      std::lock_guard l(mut);
      return std::uniform_real_distribution<float>(0.0f, std::exp(value[name] * 4) * weight)(rng);
    }
  ...
  struct BanditResultCounter {
    ...
    BanditResultCounter(Bandit& b, std::string name) : b(b), name(std::move(name)) {}
  ...
  void requestModelStateDict(std::string modelId, int modelVersion) {
    try {
      ...
      if ((rdmaHost || createRdmaHost()) && (rdmaValue >= 0.75f || (rdmaValue >= 0.0f && rpcValue < 0.5f) || bandit.sample("rdma", 4.0f) > bandit.sample("rpc"))) {
      //if (false) {
 ...
 public:
  ...
  void sendTrainData(const std::unordered_map<std::string, torch::Tensor>& data) {
    try {
...
void Server::setOnTrainData(std::function<void(std::unordered_map<std::string, torch::Tensor>)> onTrainData) {
...
void Server::updateModel(const std::string& id, std::unordered_map<std::string, torch::Tensor> stateDict) {
...
void Client::setOnUpdateModel(std::function<void (std::string_view, std::unordered_map<std::string, torch::Tensor>)> onUpdateModel) {
...
void Client::sendTrainData(const std::unordered_map<std::string, torch::Tensor>& data) {
...
void Client::sendResult(float reward, std::unordered_map<std::string_view, float> models) {
