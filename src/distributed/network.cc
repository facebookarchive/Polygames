...
template<typename T>
struct Wrapper : Ref<Wrapper<T>> {
  Cache<Wrapper<T>>* owner = nullptr;
  T obj;
  template<typename... A>
  Wrapper(Cache<Wrapper<T>>* owner, A&&... args) : owner(owner), obj(std::forward<A>(args)...) {}
  T* operator->() {
    return &obj;
  }
  T& operator*() {
...
struct Buffer : Ref<Buffer<maxsize>> {
  Cache<Buffer>* owner = nullptr;
  std::array<char, maxsize> buf;
  size_t begin = 0;
  size_t end = 0;
  Buffer(Cache<Buffer>* owner) : owner(owner) {}
  size_t space() {
    return buf.size() - end;
  }
...
std::pair<std::string_view, int> decodeEndpoint(std::string_view endpoint) {
  ...
  if (cpos != std::string_view::npos) {
    if (hostname == endpoint) hostname = endpoint.substr(0, cpos);
    ++cpos;
...
public:
  ...
  std::mutex mutex;
  PeerImpl(Cache<PeerImpl>* owner, asio::io_context& context) : owner(owner), context(context), socket(context) {}
  void connect(std::string_view endpoint) {
    auto l = lock();
    if (connected) return;
    if (endpoint.empty()) return;
    if (connectEndpoint != endpoint) connectEndpoint = endpoint;
    ...
    if (ec) {
      ...
      (*resolver)->async_resolve(remoteHost, "", [this, str = std::string(remoteHost), peer = ref(), resolver, port](const asio::error_code& ec, asio::ip::tcp::resolver::results_type results) mutable {
        if (!ec) {
          int n = 0;
          for (auto ep : results) {
            auto timer = timerCache.make(context);
            (*timer)->expires_from_now(std::chrono::seconds(n));
            (*timer)->async_wait([ep, timer, peer, port](const asio::error_code& ec) {
              if (!ec) {
                peer->connect({ep.endpoint().address(), (unsigned short)port});
              }
            });
            ++n;
          }
    ...
    (*timer)->async_wait([this, timer, peer = ref()](const asio::error_code& ec) {
      if (!ec) {
  ...
  void asyncRead(size_t offset = 0) {
    socket.async_receive(asio::buffer(readBuffer.data() + offset, readBuffer.size() - offset), [this, peer = ref()](auto&&... args) mutable {
  ...
  void setConnected(asio::ip::tcp::socket sock) {
    ...
    if (onReceiveCallback || onMessageCallback) asyncRead();
    flush();
  }
  ...
  void connect(asio::ip::tcp::endpoint ep) {
    ...
    (*h)->async_connect(ep, [this, ep, h, peer = ref()](const asio::error_code& ec) {
      auto l = lock();
      if (!ec && !connected) {
        setConnected(std::move(**h));
      } else if (ec) {
        printf("connect(%s:%d): %s\n", ep.address().to_string().c_str(), (int)ep.port(), ec.message().c_str());
      }
    });
  ...
  void callSend(std::vector<char> buffer, size_t offset) {
    ...
    socket.async_send(ab, [this, peer = ref(), buffer = std::move(buffer), offset](const asio::error_code& ec, size_t n) mutable {
      if (ec) {
        sending = false;
        failure();
      } else {
...
//      sendBuffers.push_back(asio::buffer(writeBuffer->data(), writeBuffer->size()));
//      freeN = writeBuffer->size();
//    }
//    if (!sendBuffers.empty()) {
//      sending = true;
//      socket.async_send(sendBuffers, [this, freeSrc = writeBuffer->ref(), freeN, peer = ref()](const asio::error_code& ec, size_t n) {
//        sending = false;
//        freeSrc->free(freeN);
//        if (ec) {
//          failure();
...
//  }
  ...
  struct CallbackCounter {
    std::atomic_int& c;
    CallbackCounter(std::atomic_int& c) : c(c) {
      ++c;
    }
    ~CallbackCounter() {
      --c;
  ...
  void setOnReceive(std::function<void(const void*, size_t)> callback, size_t bufferSize = 0x10000) {
    auto l = lock();
  ...
  void setOnMessage(std::function<void(const void*, size_t)> callback, size_t bufferSize = 0x10000) {
    auto l = lock();
      ...
      while (true) {
        if (messageState == 0) {
          ...
          if (messageReceived >= 4 + messageLength) {
            std::vector<char> tmp(readBuffer.data() + 4, readBuffer.data() + 4 + messageLength);
            if (messageReceived == 4 + messageLength) {
              messageState = 0;
              messageReceived = 0;
              asyncRead(messageReceived);
              ...
              std::memmove(readBuffer.data(), readBuffer.data() + 4 + messageLength, messageReceived - (4 + messageLength));
...
public:
  ...
  std::mutex mutex;
  ServerImpl(Cache<ServerImpl>* owner, asio::io_context& context) : owner(owner), context(context) {}
  ~ServerImpl() {
    close();
  }

  void asyncAccept(Handle<Wrapper<asio::ip::tcp::acceptor>> h, Handle<Wrapper<asio::ip::tcp::socket>> socket) {
    (*h)->async_accept(**socket, [this, server = ref(), h, socket](const asio::error_code& ec) {
      if (!ec) {
        if (onPeer) {
  ...
  void bind(asio::ip::tcp::endpoint ep) {
    auto retry = [&](asio::error_code ec) {
      ...
      auto timer = timerCache.make(context);
      (*timer)->expires_from_now(std::chrono::seconds(30));
      (*timer)->async_wait([this, ep, timer, server = ref()](const asio::error_code& ec) {
        if (!ec) {
          bind(ep);
        }
      });
  ...
  void bind(std::string_view endpoint) {
    ...
    if (ec) {
      auto resolver = resolverCache.make(context);
      (*resolver)->async_resolve(hostname, "", [this, peer = ref(), resolver, port](const asio::error_code& ec, asio::ip::tcp::resolver::results_type results) mutable {
        if (!ec) {
          for (auto ep : results) {
      ...
      context.post([this, address, port, server = ref()]{bind({address, (unsigned short)port});});
    }
...
public:
  ...
  static std::unique_ptr<T, std::function<void(T*)>> wrap(Handle<T> h) {
    auto* ptr = &*h;
    return std::unique_ptr<T, std::function<void(T*)>>(ptr, [h = std::move(h)](T* ptr) mutable {
      h = nullptr;
    });
  }
...
Peer::Peer(std::unique_ptr<PeerImpl, std::function<void(PeerImpl*)>> impl) : impl(std::move(impl)) {}
...
void Peer::setOnMessage(std::function<void(std::string_view)> callback) {
  if (!callback) setOnMessage(nullptr);
  else setOnMessage([callback = std::move(callback)](const void* data, size_t n) {
    return callback(std::string_view((const char*)data, n));
  });
}
...
Server::Server(std::unique_ptr<ServerImpl, std::function<void(ServerImpl*)>> impl) : impl(std::move(impl)) {}
...
Network::Network(std::unique_ptr<NetworkImpl, std::function<void(NetworkImpl*)>> impl) : impl(std::move(impl)) {}
