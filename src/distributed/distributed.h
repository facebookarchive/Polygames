...
template <typename X> void serialize(X& x, const torch::Tensor& v) {
  ...
  x(v.scalar_type(), std::basic_string_view<int64_t>(v.sizes().data(), v.sizes().size()));
...
public:
  ...
  void setOnTrainData(std::function<void(const std::unordered_map<std::string, torch::Tensor>)>);
  void start(std::string endpoint);
  void updateModel(const std::string& id, std::unordered_map<std::string, torch::Tensor> stateDict);
};
...
public:
  ...
  void setOnUpdateModel(std::function<void(std::string_view, std::unordered_map<std::string, torch::Tensor>)>);
  ...
  void sendResult(float reward, std::unordered_map<std::string_view, float> models);
};
