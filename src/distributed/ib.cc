...
//    printf("port gid_tbl_len is %d\n", attr.gid_tbl_len);
//    for (int i = 0; i != attr.gid_tbl_len; ++i) {
//      if (ibv_query_gid(context, num, i, (ibv_gid*)&gid)) {
//        throw Error("ibv_query_gid failed");
//      }
//      printf("gid %d is %s\n", i, gidstr(gid).c_str());
//      printf("interface_id %#llx\n", ((ibv_gid&)gid).global.interface_id);
//      printf("subnet_prefix %#llx\n", ((ibv_gid&)gid).global.subnet_prefix);
//    }
    if (ibv_query_gid(context, num, 0, (ibv_gid*)&gid)) {
      throw Error("ibv_query_gid failed");
    }
    //printf("returning gid %s\n", gidstr(gid).c_str());
    return gid;
  }
};
...
struct CompletionQueue: NoMove {
  ...
  ~CompletionQueue() {
    if (cq) {
      ...
      if (e) {
        throw std::runtime_error("ibv_destroy_cq failed with error " + std::to_string(e));
      }
      cq = nullptr;
    }
  }
    ...
    while (true) {
      ...
      if (r > 0) {
        ...
        double t = std::chrono::duration_cast<
                       ...
                       .count();
        printf("cq wait succeeded in %gms\n", t);
        return;
      } else if (r < 0) {
...
//    cq = ibv_create_cq_ex(ctx.context, &attr);
//    printf("created cq %p\n", cq);
//    if (!cq) {
//      throw Error("Failed to create completion queue");
//    }
...
//    auto start = std::chrono::steady_clock::now();
//    while (true) {
//      int r = ibv_start_poll(cq, &attr);
//      if (r == 0) {
//        ibv_end_poll(cq);
//        if (cq->status != IBV_WC_SUCCESS) {
//          throw Error("Failed with status: " +
//                      std::string(ibv_wc_status_str(cq->status)));
//        }
//        double t = std::chrono::duration_cast<
//                       std::chrono::duration<double, std::ratio<1, 1000>>>(
//                       std::chrono::steady_clock::now() - start)
//                       .count();
//        printf("poll succeeded in %gms\n", t);
//        return;
//      }
//      if (r != ENOENT) {
//        throw Error("Failed to poll the completion queue; error " + std::to_string(r));
//      }
//      if (std::chrono::steady_clock::now() - start >= std::chrono::milliseconds(10000)) {
//        throw Error("wait timed out");
//      }
//    }
...
struct QueuePair: NoMove {
  ...
  void rtr(const Port& port, uint16_t remoteLid, uint32_t remoteQPNum, const std::array<std::byte, 16>& gid) {
    ibv_qp_attr attr;
    memset(&attr, 0, sizeof(attr));
    ...
    //printf("connecting to gid '%s'\n", gidstr(gid).c_str());

    //printf("connecting to dev %#llx %#llx\n", ((ibv_gid&)gid).global.interface_id, ((ibv_gid&)gid).global.subnet_prefix);
  ...
  void read(MemoryRegion& dstmr,
    ...
    //printf("performing read on qp with cq %p %p\n", qp->recv_cq, qp->send_cq);
...
struct ibCompletionQueue : CompletionQueue {
  ...
  ibCompletionQueue(ib::Context& ctx, int size) : cq(ctx, size) {
  }
...
struct ibHost : Host {
  ...
  virtual void read(Buffer& localBuffer,
    ...
    if (auto buf = dynamic_cast<ibBuffer*>(&localBuffer)) {
      ...
      qp->read(buf->mrFor(Endpoint{port->lid(), qp->num(), port->gid()}), localAddress, remoteKey, remoteAddress, size);
    }
  }
  virtual void wait() override {
    cq->wait();
...
struct ibContext : Context {
  ...
  ibContext(ib::Device device) : device(device) {
...
struct ibMultiHost : ibHost {
  size_t index;
  ibMultiHost(size_t index, ib::Context* context, ib::ProtectionDomain* pd) : ibHost(context, pd), index(index) {}
...
struct ibMultiContext : Context {
  ...
  virtual std::unique_ptr<Host> createHost() override {
    size_t index = std::uniform_int_distribution<size_t>(0, contexts.size() - 1)(rng);
  ...
  virtual std::unique_ptr<Buffer> createBuffer(void* address,
    ...
    for (auto& ctx : contexts) {
      r->mrs.emplace_back(
          *ctx.pd, address, size, IBV_ACCESS_LOCAL_WRITE | IBV_ACCESS_REMOTE_READ);
